...
def runscan():
    ...
    for org in ast.literal_eval(settings.ORG_GUID):
        ...
        # breakpoint ()
        spaces = {'webops-dev': '5e8c91d6-13ec-404a-9c3e-561a37be8842', 'activity-stream-dev': 'a3750e64-2ad6-4117-afb5-1d4d64fdf80a'}
        print(spaces)
        # breakpoint()
                    ...
                    if logdrain_bound is False:
                        ...
                        if not app["name"].startswith("__conduit"):
                            ...
                            Applications.objects.update_or_create(
                                app_name=app["name"], defaults={"spaces_id": Spaces.objects.get(space_guid=spaces[space_name]).id}
                            )
